...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr.equals("unit2/output")) {
    ...
    Serial.print(game_ended);
    Serial.println();
  } else if (topicStr.equals("unit2/multiplayer/output")) {
      StaticJsonDocument<256> doc;
      deserializeJson(doc, payload, length);
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("Unit2")) {
      Serial.println("connected");
      // Subscribe
      client.subscribe("unit2/output");
      client.subscribe("unit2/multiplayer/output");
    } else {
